...
bool onSec1Timer(void *)
{
  ...
  Logger.log(LOGID, ELOG_LEVEL_ERROR, "Error log test");
  Logger.log(LOGID, ELOG_LEVEL_WARNING, "Warning log test");
  Logger.log(LOGID, ELOG_LEVEL_INFO, "Info log test: Value: %d", 42);
  Logger.log(LOGID, ELOG_LEVEL_DEBUG, "Debug log test");
  ESP_LOGV(TAG, "Verbose log test");
  log_e("Error log test with LOG_ERROR");
  log_w("Warning log test with LOG_WARNING");
  log_i("Info log test with LOG_INFO: Value: %d", 42);
  log_d("Debug log test with LOG_DEBUG");
  log_v("Verbose log test with LOG_VERBOSE");
...
void applySyslogLogLevel(const char *level)
{
  if (strcmp(level, "ERROR") == 0)
    Logger.setSyslogLogLevel(LOGID, ELOG_LEVEL_ERROR, ELOG_FAC_USER);
  else if (strcmp(level, "WARNING") == 0)
    Logger.setSyslogLogLevel(LOGID, ELOG_LEVEL_WARNING, ELOG_FAC_USER);
  else if (strcmp(level, "INFO") == 0)
    Logger.setSyslogLogLevel(LOGID, ELOG_LEVEL_INFO, ELOG_FAC_USER);
  else if (strcmp(level, "DEBUG") == 0)
    Logger.setSyslogLogLevel(LOGID, ELOG_LEVEL_DEBUG, ELOG_FAC_USER);
  else
    Logger.setSyslogLogLevel(LOGID, ELOG_LEVEL_INFO, ELOG_FAC_USER); // default
...
void setup()
{
  ...
  // Test Dump elog
  Logger.configureSyslog("123.123.123.123", 514, iotWebConf.getThingName(), true); // Syslog server IP, port and device name
  Logger.registerSyslog(LOGID, ELOG_LEVEL_DEBUG, ELOG_FAC_USER, TAG);          // Register syslog with the same device name
  Logger.log(LOGID, ELOG_LEVEL_INFO, "blabla");
